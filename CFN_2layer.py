  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.variable_scope(scope or "LSTM"):
        ...
        with tf.variable_scope("Gates"):
            ...
            b_i = tf.get_variable('b_i', [self._hidden_size], initializer=tf.ones(-self.init_scale, self.init_scale))
            ...
            b_f = tf.get_variable('b_f', [self._hidden_size], initializer=tf.ones(-self.init_scale, self.init_scale))
...
class CFN_RNNCell(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    #with tf.variable_scope(scope or "Simple_RNN"):
        W_theta = tf.get_variable('W_theta', [self._hidden_size, self._hidden_size], initializer=tf.random_uniform_initializer(-self.init_scale, self.init_scale))
        U_theta = tf.get_variable('U_theta', [self._hidden_size, self._hidden_size], initializer=tf.random_uniform_initializer(-self.init_scale, self.init_scale))
        b_theta = tf.get_variable('b_theta', [self._hidden_size], initializer=tf.random_uniform_initializer(-self.init_scale, self.init_scale))
        theta = tf.sigmoid(tf.matmul(inputs, W_theta) + tf.matmul(h, U_theta) + b_theta)
        W_eta = tf.get_variable('W_eta', [self._hidden_size, self._hidden_size], initializer=tf.random_uniform_initializer(-self.init_scale, self.init_scale))
        U_eta = tf.get_variable('U_eta', [self._hidden_size, self._hidden_size], initializer=tf.random_uniform_initializer(-self.init_scale, self.init_scale))
        b_eta = tf.get_variable('b_eta', [self._hidden_size], initializer=tf.random_uniform_initializer(-self.init_scale, self.init_scale))
        eta = tf.sigmoid(tf.matmul(inputs, W_eta) + tf.matmul(h, U_eta) + b_eta)
        W = tf.get_variable('W', [self._hidden_size, self._hidden_size], initializer=tf.random_uniform_initializer(-self.init_scale, self.init_scale))
        b = tf.get_variable('b', [self._hidden_size], initializer=tf.random_uniform_initializer(-self.init_scale, self.init_scale))
        Wx = tf.tanh(tf.matmul(inputs, W))
        output = theta*tf.tanh(h) + eta*tf.tanh(Wx)
        new_state = LSTMStateTuple(Wx, output)
