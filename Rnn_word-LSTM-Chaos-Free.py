  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.variable_scope(scope or "LSTM"):
        ...
        with tf.variable_scope("Gates"):
            ...
            W_i = tf.constant([[-1.0, -4.0], [-3.0, -2.0]])
            i = tf.sigmoid(tf.matmul(h, W_i))
            W_o = tf.constant([[4.0, 1.0], [-9.0, -7.0]])
            o = tf.sigmoid(tf.matmul(h, W_o))
            W_f = tf.constant([[-2.0, 6.0], [0.0, -6.0]])
            f = tf.sigmoid(tf.matmul(h, W_f))
            #o = tf.sigmoid(tf.matmul(state, U_o) + b_o)
            
        with tf.variable_scope("Candidate"):
            W_g = tf.constant([[-1.0, -6.0], [6.0, -9.0]])
            g = tf.tanh(tf.matmul(h, W_g))
        
        #mt = o*tf.tanh(c)
        #W_y = tf.get_variable('W_y', [self._hidden_size, self._hidden_size], initializer=tf.random_uniform_initializer(-init_scale, init_scale))
        #b_y = tf.get_variable('b_y', [self._hidden_size], initializer=tf.random_uniform_initializer(-init_scale, init_scale))
        
        new_c = tf.multiply(c, f) + tf.multiply(i, g)
        new_h = tf.multiply(tf.tanh(new_c), o)
...
def run_epoch(sess, m_train, raw_data, train_op, config, is_train=False, lr=None):
  ...
  for (batch_x, batch_y) in batches:
    ...
    #if is_train:
    c, state_val = sess.run(
        [m_train.cost, m_train.state],
